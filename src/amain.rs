...
use eframe::egui;
use egui_wgpu::{self, wgpu::core::command::RenderBundle};
...
impl Custom3d {
    pub fn new<'a>(cc: &'a eframe::CreationContext<'a>) -> Self {
        ...
        // let render_state = cc.as_ref().expect("WGPU enabled");

        let device = cc.egui_ctx.device;
...
fn main() {
    ...
    eframe::run_native(
        ...
        options,
        Box::new(|cc| {
            Box::<Custom3d>::new(Custom3d{angle: 45.0})
        }),
    ).unwrap();
}