...
fn frag_main(@location(0) texcoord: vec2<f32>) -> @location(0) vec4<f32> {
    var radius: f32 = 10.0; // Adjust for desired blur radius
    var sigma: f32 = radius / 3.0;
    var kernelSize: i32 = i32(ceil(radius) * 2.0 + 1.0);
    ...
    for (var offsetX : i32 = -kernelSize / 2; offsetX <= kernelSize / 2; offsetX++) {
        ...
        var weight: f32 = exp(-(f32(offsetX) * f32(offsetX)) / (2.0 * sigma * sigma)) / (sqrt(2.0 * 3.14159) * sigma);
    ...
    for (var offsetY : i32 = -kernelSize / 2; offsetY <= kernelSize / 2; offsetY++) {
        ...
        var weight: f32 = exp(-(f32(offsetY) * f32(offsetY)) / (2.0 * sigma * sigma)) / (sqrt(2.0 * 3.14159) * sigma);
        result += textureSample(inputTexture, sampler0, samplePos) * weight;
        sum += weight;
    }
    result /= sum;
