...
fn main(@builtin(position) FragCoord : vec4<f32>) -> @location(0) vec4<f32> {
    ...
    for (var offsetX : i32 = -kernelSize / 2; offsetX <= kernelSize / 2; offsetX++) {
        let samplePos : vec2<f32> = FragCoord.xy / textureSize + vec2<f32>(f32(offsetX), 0.0) / textureSize;  // Horizontal pass

        let weight : f32 = exp(-(float(offsetX) * float(offsetX)) / (2.0 * sigma * sigma)) / (sqrt(2.0 * 3.14159) * sigma);
